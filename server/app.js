...
// SSH 접속 정보를 URL Context 로 전달 받음
app.get('/ssh/:guid/:instance/:host/:port/:sshCode/', function (req, res, next) {
		
	let username = "cf:" + req.params.guid + "/" + req.params.instance + "@" + req.params.host;
	
	req.session.username = username;
    req.session.userpassword = req.params.sshCode;
	
	app.use(myutil.basicAuth)
	
	responseSSHConnection(req, res, next)
})

// ssh 화면 전달 및 SSH Connection
function responseSSHConnection(req, res, next) {
  ...
  req.session.ssh = {
    ...
    port: (validator.isInt(req.query.port + '', { min: 1, max: 65535 }) && req.query.port) 
	  || req.params.port
	  || config.ssh.port,
